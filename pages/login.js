...
<Layout className={styles.layout} >
    ...
    <div className={styles.logo}><a href="/"><Image src="/moviebook_clear.png" width="170" height="64"/></a></div>
      <Menu  mode="horizontal" defaultSelectedKeys={['2']} style={{float: 'right', marginRight: '40px'}}>
        <Menu.Item key="1"><a href="/PREsignup">Registrati</a></Menu.Item>
        <Menu.Item key="2"><a href="#">Accedi</a></Menu.Item>
      </Menu>
    </Header>
    ...
    onFinish = {(data) => {
      ...
      if (data.Negozio) {
        ...
        console.log(businessUsers)
         if (businessUsers.find((el) => el.email === data.email && el.password === data.password)) {
          localStorage.setItem(
            'user',
            JSON.stringify({
              type: 'businessUser',
              info: businessUsers.filter((el) => el.email === data.email)[0],
            }),
          );
          window.location.href="/business/BusinessHomePage";
      } else {
        ...
        if (privateUsers.find((el) => el.email === data.email && el.password === data.password)) {
          localStorage.setItem(
           'user',
            JSON.stringify({
             type: 'privateUser',
              info: privateUsers.filter((el) => el.email === data.email)[0],
            }),
          );
          window.location.href="/user/UserHomePage";
        } else {
